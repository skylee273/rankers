...
class LeagueAdapter(val responseList: MutableList<LeagueItem>?) : RecyclerView.Adapter<BaseViewHolder>() {
    ...
    inner class LeagueViewHolder(private val mBinding: ItemLeagueViewBinding) : BaseViewHolder(mBinding.root), LeagueItemViewModel.LeagueItemViewModelListener {
        ...
        override fun onRemove0Click() {
            val position = adapterPosition
            if(responseList!![position].player1.isNotEmpty())
                mListener!!.onAddPeopleDialog(position, 1, responseList!![position].player1)
        }

        override fun onRemove1Click() {
            val position = adapterPosition
            if(responseList!![position].player2.isNotEmpty())
                mListener!!.onAddPeopleDialog(position, 2, responseList!![position].player2)
        }

        override fun onRemove2Click() {
            val position = adapterPosition
            if(responseList!![position].player3.isNotEmpty())
                mListener!!.onAddPeopleDialog(position, 3, responseList!![position].player3)
        }

        override fun onRemove3Click() {
            val position = adapterPosition
            if(responseList!![position].player4.isNotEmpty())
                mListener!!.onAddPeopleDialog(position, 4, responseList!![position].player4)
        }

        override fun onPlayer1Click() {
            Log.d("onPlayer1Click","클릭")
            val position = adapterPosition
            if(responseList!![position].player1.isEmpty())
                mListener!!.onShowPeopleDialog(position,1)
        }

        override fun onPlayer2Click() {
            val position = adapterPosition
            if(responseList!![position].player2.isEmpty())
                mListener!!.onShowPeopleDialog(position,2)
        }

        override fun onPlayer3Click() {
            val position = adapterPosition
            if(responseList!![position].player3.isEmpty())
                mListener!!.onShowPeopleDialog(position,3)
        }

        override fun onPlayer4Click() {
            val position = adapterPosition
            if(responseList!![position].player4.isEmpty())
    ...
    interface LeagueAdapterListener {
        ...
        fun onAddPeopleDialog(position : Int, type : Int, playerName: String)
        fun onRetryClick()
    }
