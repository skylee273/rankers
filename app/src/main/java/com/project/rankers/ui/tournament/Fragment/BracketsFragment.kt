package com.project.rankers.ui.tournament.Fragment
...
import com.project.rankers.ui.tournament.adapter.BracketsSectionAdapter
import com.project.rankers.ui.tournament.customviews.WrapContentHeightViewPager
import com.project.rankers.ui.tournament.model.ColomnData
import com.project.rankers.ui.tournament.model.CompetitorData
import com.project.rankers.ui.tournament.model.MatchData
import com.project.rankers.ui.tournament.utility.BracketsUtility
                ...
                .subscribe({ response: GroupResponse ->
                    for (item in response.items) {
                        if (item.groupRank1!!.toInt() == 1 || item.groupRank1!!.toInt() == 2) {
                            tournamentPeopleList!!.add(item.groupPlayer1!!)
                        }
                        if (item.groupRank2!!.toInt() == 1 || item.groupRank2!!.toInt() == 2) {
                            tournamentPeopleList!!.add(item.groupPlayer2!!)
                        }
                        if (item.groupRank3!!.toInt() == 1 || item.groupRank3!!.toInt() == 2) {
                            tournamentPeopleList!!.add(item.groupPlayer3!!)
                        }
                        if (item.groupRank4!!.toInt() == 1 || item.groupRank4!!.toInt() == 2) {
                            tournamentPeopleList!!.add(item.groupPlayer4!!)
                        }
                    }
                    setData()
        ...
        if (mCurrentPagerState != ViewPager.SCROLL_STATE_SETTLING) {
            ...
            if (positionOffset > 0.5) {
                ...
                if (position + 1 != mNextSelectedScreen) {
                    ...
                    //update view here
                    if (getBracketsFragment(position)!!.colomnList[0].height != BracketsUtility.dpToPx(131))
                        getBracketsFragment(position)!!.shrinkView(BracketsUtility.dpToPx(131))
                    if (getBracketsFragment(position + 1)!!.colomnList[0].height != BracketsUtility.dpToPx(131))
